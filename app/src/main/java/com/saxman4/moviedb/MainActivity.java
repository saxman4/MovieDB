...
public class MainActivity extends AppCompatActivity
        implements MovieFragment.MovieFragmentListener,
    ...
    private MovieFragment movieFragment; // displays movie list

    // display MovieFragment when MainActivity first loads
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // create and display a MovieFragment
        if (savedInstanceState == null &&
                findViewById(R.id.fragmentContainer) != null) {
            // create MovieFragment
            movieFragment = new MovieFragment();

            // add the fragment to the FrameLayout
            FragmentTransaction transaction =
                    getSupportFragmentManager().beginTransaction();
            transaction.add(R.id.fragmentContainer, movieFragment);
            transaction.commit(); // display MovieFragment
        }
        else {
            movieFragment =
                    (MovieFragment) getSupportFragmentManager().
                            findFragmentById(R.id.movieFragment);
        }
    }
    ...
    public void onMovieDeleted() {
        // removes top of back stack
        getSupportFragmentManager().popBackStack();
        movieFragment.updateMovieList(); // refresh movies
    }
